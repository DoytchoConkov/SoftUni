...
#9
SELECT concat_ws(' ',`first_name`,`middle_name`,`last_name`) AS `full_name` FROM `employees` WHERE `salary` IN (25000, 14000, 12500 , 23600);
...
#23
SELECT `country_name`,`country_code`,IF(`currency_code`='EUR','EUR','Not EUR') AS `currency` FROM `countries` ORDER BY `country_name`;
